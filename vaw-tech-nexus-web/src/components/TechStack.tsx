    ...
    <div className="container mx-auto px-4">
        ...
        {techItems.map((tech, index) => <Card key={index} className={`tech-card hover:-translate-y-2 transition-all duration-300 ${index === animatedIndex ? 'ring-2 ring-tech-purple/50' : ''}`}>
          <AspectRatio ratio={1 / 1} className="w-full">
            <CardContent className="flex flex-col items-center p-4 text-center h-full justify-center">
              <div className="mb-2 bg-muted/30 p-3 rounded-full flex items-center justify-center w-16 h-16">
                {tech.iconUrl ? (
                  <img
                    src={tech.iconUrl}
                    alt={tech.name}
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-6">
          {techLogos.map((tech, index) => <div key={index} className="flex flex-col items-center justify-center p-3 rounded-lg hover:bg-muted/20 transition-all duration-300 group">
            <div className="h-12 md:h-14 w-12 md:w-14 flex items-center justify-center mb-2 group-hover:scale-110 transition-transform duration-300">
              <img src={tech.logo} alt={tech.name} className="max-h-12 md:max-h-14 w-auto object-contain filter grayscale group-hover:grayscale-0 transition-all duration-300" onError={e => {
