...
**Action:** Always avoid putting ticking state values in interval-establishing `useEffect` dependency arrays when functional state updates can compute the next value.
## 2026-08-03 - Memoizing Derived Status Counts in Task Management
**Learning:** Performing multiple `.filter()` iterations on arrays inside render cycles (e.g., repeatedly iterating over all tasks to count pending, completed, etc.) causes $O(C \times N)$ time complexity and degrades rendering performance as array sizes grow.
**Action:** Always wrap high-frequency derived state calculations or aggregations in a `useMemo` block, and combine them into a single $O(N)$ iteration loop to minimize processing overhead.
