...
const ClientLogo: React.FC<ClientLogoProps> = ({
  ...
  return <div className="flex items-center justify-center px-8">
      <img src={src} alt={alt} className="h-12 w-auto object-contain transition-all duration-300 transform hover:scale-110" />
    </div>;
};
const ClientLogos = () => {
  const clients = [{
  ...
  return <section className="relative overflow-hidden my-0 px-0 py-0">
        ...
        <div className="relative overflow-hidden before:absolute before:left-0 before:top-0 before:z-10 before:w-24 before:h-full before:bg-gradient-to-r before:from-background before:to-transparent after:absolute after:right-0 after:top-0 after:z-10 after:w-24 after:h-full after:bg-gradient-to-l after:from-background after:to-transparent">
          <div className="logo-carousel">
            <div className="logo-slide">
              {clients.map((client, index) => <ClientLogo key={`original-${index}`} src={client.logo} alt={client.name} />)}
            </div>
            <div className="logo-slide">
              {clients.map((client, index) => <ClientLogo key={`duplicate-${index}`} src={client.logo} alt={client.name} />)}
            </div>
          </div>
        </div>
