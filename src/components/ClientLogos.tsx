  ...
  return (
    <div className="flex items-center justify-center p-4 group">
      <img 
        src={src} 
        alt={alt} 
        className="h-16 w-auto object-contain grayscale hover:grayscale-0 opacity-70 group-hover:opacity-100 transition-all duration-300 transform group-hover:scale-110"
      />
    </div>
  );
};
    ...
    <section ref={containerRef} className="py-16 relative overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        <div className="relative overflow-hidden py-10 before:absolute before:left-0 before:top-0 before:z-10 before:w-24 before:h-full before:bg-gradient-to-r before:from-background before:to-transparent after:absolute after:right-0 after:top-0 after:z-10 after:w-24 after:h-full after:bg-gradient-to-l after:from-background after:to-transparent">
          <div className="flex scroller" data-direction="right" style={{animation: 'scroll 25s linear infinite'}}>
            {[...clients, ...clients].map((client, index) => (
              <ClientLogo key={index} src={client.logo} alt={client.name} />
            ))}
          </div>
          
          <div className="flex scroller mt-8" data-direction="left" style={{animation: 'scroll 25s linear infinite reverse'}}>
            {[...clients.slice().reverse(), ...clients.slice().reverse()].map((client, index) => (
              <ClientLogo key={index + 100} src={client.logo} alt={client.name} />
            ))}
          </div>
        </div>
      ...
      <div className="absolute right-20 top-10 w-16 h-16 border-2 border-tech-red/20 transform rotate-45 opacity-30 animate-float" style={{animationDelay: '0.5s'}}></div>
    </section>
  );
};
